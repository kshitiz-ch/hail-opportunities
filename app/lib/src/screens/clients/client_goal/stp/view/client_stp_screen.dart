import 'package:app/src/config/constants/color_constants.dart';
import 'package:app/src/config/constants/enums.dart';
import 'package:app/src/config/constants/string_constants.dart';
import 'package:app/src/config/routes/router.gr.dart';
import 'package:app/src/config/utils/function_utils.dart';
import 'package:app/src/controllers/client/goal/stp_controller.dart';
import 'package:app/src/screens/commons/empty_screen/empty_screen.dart';
import 'package:app/src/widgets/app_bar/custom_app_bar.dart';
import 'package:app/src/widgets/bottomsheet/confirm_send_ticket_bottomsheet.dart';
import 'package:app/src/widgets/button/action_button.dart';
import 'package:app/src/widgets/misc/common_ui.dart';
import 'package:auto_route/auto_route.dart';
import 'package:core/modules/clients/models/client_list_model.dart';
import 'package:core/modules/mutual_funds/models/goal_model.dart';
import 'package:core/modules/mutual_funds/models/user_goal_subtype_scheme_model.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';

import '../widgets/add_edit_fund_bottomsheet.dart';
import '../widgets/stp_basket_list.dart';

@RoutePage()
class ClientStpScreen extends StatelessWidget {
  const ClientStpScreen({
    Key? key,
    required this.client,
    required this.goalSchemes,
    required this.goal,
    this.anyFundGoalScheme,
  }) : super(key: key);

  final Client client;
  final List<UserGoalSubtypeSchemeModel> goalSchemes;
  final GoalModel goal;
  final UserGoalSubtypeSchemeModel? anyFundGoalScheme;

  @override
  Widget build(BuildContext context) {
    return GetBuilder<StpController>(
      init: StpController(
        client: client,
        goal: goal,
        goalSchemes: goalSchemes,
        anyFundGoalScheme: anyFundGoalScheme,
      ),
      builder: (controller) {
        return Scaffold(
          backgroundColor: ColorConstants.white,
          appBar: CustomAppBar(
            titleText: 'Systematic Transfer Plan (STP)',
          ),
          body: Container(
            child: Builder(
              builder: (context) {
                if (controller.stpBasket.isNotEmpty) {
                  return StpBasketList();
                }

                return _buildEmptyState(context, controller);
              },
            ),
          ),
          floatingActionButtonLocation:
              FloatingActionButtonLocation.centerDocked,
          floatingActionButton: _buildActionButton(context, controller),
        );
      },
    );
  }

  Widget _buildEmptyState(BuildContext context, StpController controller) {
    return Center(
      child: EmptyScreen(
        message:
            "No Funds ${controller.switchOutSchemes.isEmpty ? 'Found' : 'Selected'}",
        actionButtonText:
            controller.switchOutSchemes.isNotEmpty ? '+ Add Funds' : null,
        onClick: () {
          _navigateToStpSchemeForm(context, controller);
        },
      ),
    );
  }

  Widget _buildActionButton(BuildContext context, StpController controller) {
    if (controller.stpBasket.isEmpty) {
      return SizedBox();
    }

    return Container(
      padding: EdgeInsets.symmetric(vertical: 10),
      decoration: BoxDecoration(
        color: ColorConstants.white,
        border: Border(
          top: BorderSide(color: ColorConstants.borderColor),
        ),
      ),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        mainAxisAlignment: MainAxisAlignment.end,
        children: [
          ClickableText(
            text: '+ Add More Funds',
            fontSize: 16,
            onClick: () {
              _navigateToStpSchemeForm(context, controller);
            },
          ),
          SizedBox(height: 16),
          ActionButton(
            onPressed: () {
              CommonUI.showBottomSheet(
                context,
                child: GetBuilder<StpController>(
                  id: GetxId.sendTicket,
                  builder: (controller) {
                    return ConfirmSendTicketBottomSheet(
                      viaProposal: true,
                      title: 'Systematic Transfer Plan (STP)',
                      isLoading: controller.stpOrderResponse.state ==
                          NetworkState.loading,
                      onConfirm: () async {
                        await controller.createStpOrder();

                        if (controller.stpOrderResponse.state ==
                            NetworkState.loaded) {
                          AutoRouter.of(context).push(
                            ProposalSuccessRoute(
                              client: controller.client,
                              productName: 'Create Stp',
                              proposalUrl:
                                  controller.ticketResponse?.customerUrl,
                            ),
                          );
                        }

                        if (controller.stpOrderResponse.state ==
                            NetworkState.error) {
                          return showToast(
                            text: controller.stpOrderResponse.message,
                          );
                        }
                      },
                    );
                  },
                ),
              );
            },
            text: 'Send Proposal',
          )
        ],
      ),
    );
  }

  void _navigateToStpSchemeForm(
      BuildContext context, StpController controller) {
    // Clear previous state from bottomsheet before selecting new fund
    if (controller.dropdownSelectedScheme != null) {
      controller.clearInputStates();
    }

    // List<SchemeMetaModel> schemes = controller.isAnyFundPortfolio
    //     ? controller.anyFundSwitchOutSchemes
    //     : controller.switchOutSchemes;

    // if (schemes.length == 1) {
    //   StpSchemeContext schemeContext =
    //       StpSchemeContext(switchOut: SchemeMetaModel.clone(schemes.first));

    //   controller.amountController.clear();
    //   controller.updateDropdownSelectedScheme(schemeContext);
    // }

    CommonUI.showBottomSheet(
      context,
      child: AddEditFundBottomSheet(),
    );
  }
}
