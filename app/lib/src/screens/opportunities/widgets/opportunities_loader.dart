...
// --- 1. The Wrapper View with Text ---
class OpportunitiesLoadingView extends StatefulWidget {
  const OpportunitiesLoadingView({Key? key}) : super(key: key);

  @override
  State<OpportunitiesLoadingView> createState() =>
      _OpportunitiesLoadingViewState();
}

class _OpportunitiesLoadingViewState extends State<OpportunitiesLoadingView>
    with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    // Create a breathing animation for the text
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(
        ...
        children: [
          // The Compass Loader we built earlier
          const OpportunitiesLoader(
            size: 60,
            ...
            child: Column(
              children: [
                const Text(
                  ...
                  style: TextStyle(
                    color: Color(0xFF7F30FE), // Primary Purple
                    fontSize: 16,
                ...
                Text(
                  "Scanning for opportunities and critical gaps",
                  style: TextStyle(
                    color: Colors.grey[500], // Subtitle in lighter grey
                    fontSize: 12,
                  ),
                ),
              ],
...
// --- 2. The Compass Loader (From previous step) ---
class OpportunitiesLoader extends StatefulWidget {
  final double size;
  final Color color;
  ...
  const OpportunitiesLoader({
    Key? key,
    this.size = 50.0,
    this.color = const Color(0xFF7F30FE),
  }) : super(key: key);
...
class _CompassPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    needlePath.lineTo(needleWid / 2, 0);
