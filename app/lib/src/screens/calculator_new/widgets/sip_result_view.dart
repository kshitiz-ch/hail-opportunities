...
class SipResultView extends StatelessWidget {
  final CalculatorController controller = Get.find<CalculatorController>();

  @override
  Widget build(BuildContext context) {
    final data = controller.calculateSIPData();

    // Calculate summary values from the sipMonthlyData
    ...
    final showMonthly = controller.investmentPeriod.value == 1;

    final chartData = data.map((entry) {
      final xLabel = showMonthly ? entry['month'] ?? 0 : entry['year'] ?? 0;
      final investment = entry['investment'] ?? 0;
      final gain = entry['gain'] ?? 0;
      return CalculatorBarChartData(
        xLabel: '${showMonthly ? 'Month' : 'Year'} ${xLabel.toInt()}',
        yValue: investment,
        yValue2: gain,
        yLabel: 'Investment',
        yLabel2: 'Gain',
